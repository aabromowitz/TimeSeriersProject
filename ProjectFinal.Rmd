...
# Zoomed In
plot(log.mhp[149:198], type = 'l', main = 'Univariate MLP 1 Year Forecast with Confidence Intervals', 
     ylim=c(min(fit.mlp$fitted)-.1,max(fit.mlp$fitted)+.1))
...
# Zoomed In
plot(log.mhp[149:198], type = 'l', main = 'Univariate MLP 5 Year Forecast with Confidence Intervals',
     ylim=c(min(fit.mlp$fitted)-.1,max(fit.mlp$fitted)+.1))
...
library(nnfor)
set.seed(30)
fit.mlp = mlp(ts(xt)) #mlp(ts(xtTrain), comb = 'median', xreg = ts.train.X.4, sel.lag = TRUE)
...
lines(seq(199,202), f.xreg.4$mean, col = "red")
