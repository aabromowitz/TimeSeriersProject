...
```{r Univariate MLP Confidence Intervals, warning=FALSE}
################################################################################
...
```{r Future Univariate MLP Confidence Intervals, warning=FALSE}
################################################################################
...
# Best 1 Year Multivariate Model Zoomed In
plot(seq(150,198,1),log.mhp[150:198], ylim = c(ymin_plot, ymax_future), type = 'l', 
     xlab = 'Time', ylab = 'log Median Housing Sales Price',
     main = "Multivariate MLP Short Term Forecast")
points(seq(195,198), forecasted.mlp.X.d1.4$mean, type = 'l', col = 'red')

# Best 5 Year Multivariate Models Zoomed In
plot(seq(150,198,1),log.mhp[150:198], ylim = c(ymin_plot, ymax_future), type = 'l', 
     xlab = 'Time', ylab = 'log Median Housing Sales Price',
     main = "Multivariate MLP Long Term Forecast")
points(seq(179,198), forecasted.mlp.X.d2.20$mean, type = 'l', col = 'red')

```
### Confidence Intervals for Multivariate Models

Though the MLP Confidence Interval code has been adjusted to include xreg in the fit and forecasts, the model forecasts way too accurately for us to be confident that it's working correctly. This may have to do with not using a training subset for the xreg variables, as its not realistic that we would have the future values of the exogenous variables. More exploration of confidence intervals for Multivariate MLP is needed.

```{r Multivariate MLP Confidence Intervals, warning=FALSE}
################################################################################

# Trying what Dr Sadler did, but for an mlp model

# make dataframe
ts.X = data.frame(hor = ts(fed_housing_data$Ownership_Rate), 
                  huc = ts(fed_housing_data$Housing_Units_Completed), 
                  snh = ts(fed_housing_data$Supply_New_Houses), 
                  hpi = ts(fed_housing_data$Housing_Price_Index))
...
xtTrain = xt[1:(l-h.short)]
ts.X.train = ts.X[1:(l-h.short),]
plotts.wge(res)

# Make the function
num_back = l - lf
BSSim = function(ts,ts.x,res,fit,num_back,horizon) {
  origin = length(ts)
  ForHolder = numeric(horizon)
  for(i in (origin+1):(origin+horizon)){
    # ts[i] = ts[i-1]*phi + sample(res,1)
...
for(i in 1:BSSamples)
{
  ...
  xtNewFor = BSSim(xtTrain,ts.X,res,fit,num_back,h.short)
  holder[i,] = xtNewFor
}
...
h.short.xtNewForBS = xtNewForBS
h.short.percentiles_per_column = percentiles_per_column
...
# plotts.wge(c(xtTrain-est$res,xtNewForBS))
plot(log.mhp[6:198], type = 'l', 
     main = 'Multivariate MLP Short Term Forecast with Confidence Intervals', 
     xlab = 'Time', ylab = 'log Median Housing Sales Price', ylim=c(min(xt),ymax_future))
...
# Zoomed In
plot(seq(150,198,1), log.mhp[150:198], type = 'l', 
     main = 'Multivariate MLP Short Term Forecast with Confidence Intervals', 
     xlab = 'Time', ylab = 'log Median Housing Sales Price', ylim=c(ymin_plot,ymax_future))
lines(seq(195,198,1),percentiles_per_column[,1], lty = 3, col = "blue")
lines(seq(195,198,1),percentiles_per_column[,2], lty = 3, col = "blue")
...
for(i in 1:BSSamples)
{
  ...
  xtNewFor = BSSim(xtTrain,ts.X,res,fit,num_back,h.long)
  holder[i,] = xtNewFor
}
...
h.long.xtNewForBS = xtNewForBS
h.long.percentiles_per_column = percentiles_per_column
...
# plotts.wge(c(xtTrain-est$res,xtNewForBS))
plot(log.mhp[6:198], type = 'l', 
     main = 'Multivariate MLP Long Term Forecast with Confidence Intervals', 
     xlab = 'Time', ylab = 'log Median Housing Sales Price', ylim=c(min(xt),ymax_future))
...
### CI for Future Forecast with Multivariate MLP

```{r CI for Future Multivariate MLP, warning=FALSE}
################################################################################
...
BSSim(xtTrain,ts.X,res,fit,num_back,h.short)
...
# Only got this far because the function started producing NAs:
holder[1,]
...
# Alex hard coded seq because lines were being plotted that didn't line up with the actual time
# plotts.wge(c(xtTrain-est$res,xtNewForBS))
#plot(seq(1,l+h.short,1),xt[1:(l+h.short)], type = 'l', 
#     main = 'Multivariate MLP Short Term Forecast', 
...
# Make the function
num_back = l - lf
BSSim = function(xtTrain,ts.x,res,fit,num_back,horizon) {
...
# Run the function
BSSamples = 500 # I'm worried 100k will take a really long time, so make it less
holder = matrix(nrow = BSSamples,ncol = h.long)
for(i in 1:BSSamples)
{
  # xtNewFor = BSSim(xtTrain,est$res,est$phi,50)
...
# Values for 1 yr and 5 yr Forecast with bootstrap method
#f.h.short.xtNewForBS[4]
#f.h.short.percentiles_per_column[4,1]
#f.h.short.percentiles_per_column[4,2]

#f.h.long.xtNewForBS[20]
#f.h.long.percentiles_per_column[20,1]
#f.h.long.percentiles_per_column[20,2]
